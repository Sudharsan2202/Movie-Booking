...
export default function MovieDetailPage() {
  ...
  // closed. Re-runs on the 30s ticker so it self-corrects as time passes. ──
  useEffect(() => {
    ...
    // start looking from whichever day is currently selected (clamped),
    // falling back to day 0 if it's out of range (e.g. data just loaded)
    const startDay =
    ...
    if (foundDayIndex === -1) {
      ...
      // clear selection, let the "no showtimes" / closed UI take over
    ...
    // only auto-set selectedTime if nothing is currently selected, or the
    // currently selected time has since closed — don't yank a still-valid
    // manual selection away from the user
    setSelectedTime((cur) => {
  ...
  useEffect(() => {
    ...
    // currently selected time just closed — find the next open one
    for (let offset = 0; offset < showtimeDays.length; offset++) {
    ...
    // nothing open anywhere — clear selection
    setSelectedTime(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tick]);
